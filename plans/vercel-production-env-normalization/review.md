...
## What was built
- Vercel production deployments now resolve `nodeEnv=production` from `VERCEL_ENV=production` even when `NODE_ENV` is missing.
...
- Diagnostics showed `nodeEnv=development`, `executionMode=direct`, valid `DATABASE_URL`, invalid quoted-empty JWT secrets, and valid maintenance secret.
