//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_import

import 'package:one_of_serializer/any_of_serializer.dart';
import 'package:one_of_serializer/one_of_serializer.dart';
import 'package:built_collection/built_collection.dart';
import 'package:built_value/json_object.dart';
import 'package:built_value/serializer.dart';
import 'package:built_value/standard_json_plugin.dart';
import 'package:built_value/iso_8601_date_time_serializer.dart';
import 'package:mag_api_client/src/date_serializer.dart';
import 'package:mag_api_client/src/model/date.dart';

import 'package:mag_api_client/src/model/add_cart_item_dto.dart';
import 'package:mag_api_client/src/model/add_wishlist_item_dto.dart';
import 'package:mag_api_client/src/model/adjust_stock_dto.dart';
import 'package:mag_api_client/src/model/admin_access_check_response_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_attribute_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_attribute_option_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_brand_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_category_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_collection_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_media_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_product_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_product_type_attribute_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_product_type_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_product_variant_option_value_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_relation_link_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_tag_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_translation_dto.dart';
import 'package:mag_api_client/src/model/admin_catalog_variant_dto.dart';
import 'package:mag_api_client/src/model/admin_customer_summary_dto.dart';
import 'package:mag_api_client/src/model/admin_legal_references_response_dto.dart';
import 'package:mag_api_client/src/model/admin_paginated_catalog_products_dto.dart';
import 'package:mag_api_client/src/model/admin_permission_dto.dart';
import 'package:mag_api_client/src/model/admin_role_dto.dart';
import 'package:mag_api_client/src/model/apply_coupon_dto.dart';
import 'package:mag_api_client/src/model/attach_media_dto.dart';
import 'package:mag_api_client/src/model/attached_media_response_dto.dart';
import 'package:mag_api_client/src/model/attached_media_translation_dto.dart';
import 'package:mag_api_client/src/model/attribute_option_dto.dart';
import 'package:mag_api_client/src/model/attribute_option_translation_dto.dart';
import 'package:mag_api_client/src/model/attribute_translation_dto.dart';
import 'package:mag_api_client/src/model/audit_log_actor_dto.dart';
import 'package:mag_api_client/src/model/audit_log_response_dto.dart';
import 'package:mag_api_client/src/model/auth_response_dto.dart';
import 'package:mag_api_client/src/model/auth_user_dto.dart';
import 'package:mag_api_client/src/model/cancel_order_dto.dart';
import 'package:mag_api_client/src/model/cart_item_response_dto.dart';
import 'package:mag_api_client/src/model/cart_response_dto.dart';
import 'package:mag_api_client/src/model/catalog_localized_reference_dto.dart';
import 'package:mag_api_client/src/model/catalog_media_summary_dto.dart';
import 'package:mag_api_client/src/model/catalog_product_summary_dto.dart';
import 'package:mag_api_client/src/model/catalog_status_response_dto.dart';
import 'package:mag_api_client/src/model/catalog_variant_option_summary_dto.dart';
import 'package:mag_api_client/src/model/catalog_variant_price_dto.dart';
import 'package:mag_api_client/src/model/catalog_variant_summary_dto.dart';
import 'package:mag_api_client/src/model/checkout_address_dto.dart';
import 'package:mag_api_client/src/model/checkout_cart_preview_dto.dart';
import 'package:mag_api_client/src/model/checkout_payment_attempt_dto.dart';
import 'package:mag_api_client/src/model/checkout_payment_readiness_dto.dart';
import 'package:mag_api_client/src/model/checkout_place_order_dto.dart';
import 'package:mag_api_client/src/model/checkout_place_order_response_dto.dart';
import 'package:mag_api_client/src/model/checkout_preview_applied_promotion_dto.dart';
import 'package:mag_api_client/src/model/checkout_preview_dto.dart';
import 'package:mag_api_client/src/model/checkout_preview_item_dto.dart';
import 'package:mag_api_client/src/model/checkout_preview_item_response_dto.dart';
import 'package:mag_api_client/src/model/checkout_preview_item_stock_dto.dart';
import 'package:mag_api_client/src/model/checkout_preview_response_dto.dart';
import 'package:mag_api_client/src/model/checkout_preview_shipping_method_dto.dart';
import 'package:mag_api_client/src/model/content_page_response_dto.dart';
import 'package:mag_api_client/src/model/coupon_response_dto.dart';
import 'package:mag_api_client/src/model/create_admin_role_dto.dart';
import 'package:mag_api_client/src/model/create_attribute_dto.dart';
import 'package:mag_api_client/src/model/create_brand_dto.dart';
import 'package:mag_api_client/src/model/create_cart_dto.dart';
import 'package:mag_api_client/src/model/create_category_dto.dart';
import 'package:mag_api_client/src/model/create_collection_dto.dart';
import 'package:mag_api_client/src/model/create_media_upload_intent_dto.dart';
import 'package:mag_api_client/src/model/create_product_dto.dart';
import 'package:mag_api_client/src/model/create_product_type_dto.dart';
import 'package:mag_api_client/src/model/create_product_variant_dto.dart';
import 'package:mag_api_client/src/model/create_report_export_dto.dart';
import 'package:mag_api_client/src/model/create_return_request_dto.dart';
import 'package:mag_api_client/src/model/create_return_request_item_dto.dart';
import 'package:mag_api_client/src/model/create_review_dto.dart';
import 'package:mag_api_client/src/model/create_shipment_dto.dart';
import 'package:mag_api_client/src/model/create_shipment_item_dto.dart';
import 'package:mag_api_client/src/model/create_tag_dto.dart';
import 'package:mag_api_client/src/model/execute_return_refund_dto.dart';
import 'package:mag_api_client/src/model/fulfillment_item_dto.dart';
import 'package:mag_api_client/src/model/fulfillment_response_dto.dart';
import 'package:mag_api_client/src/model/health_indicator_detail_dto.dart';
import 'package:mag_api_client/src/model/health_liveness_response_dto.dart';
import 'package:mag_api_client/src/model/health_readiness_response_dto.dart';
import 'package:mag_api_client/src/model/inventory_kpis_response_dto.dart';
import 'package:mag_api_client/src/model/inventory_kpis_stock_metric_dto.dart';
import 'package:mag_api_client/src/model/inventory_kpis_stock_sum_dto.dart';
import 'package:mag_api_client/src/model/invitation_sent_response_dto.dart';
import 'package:mag_api_client/src/model/invite_staff_dto.dart';
import 'package:mag_api_client/src/model/legal_references_response_dto.dart';
import 'package:mag_api_client/src/model/login_dto.dart';
import 'package:mag_api_client/src/model/logout_dto.dart';
import 'package:mag_api_client/src/model/manual_tax_rate_response_dto.dart';
import 'package:mag_api_client/src/model/media_translation_dto.dart';
import 'package:mag_api_client/src/model/media_upload_headers_dto.dart';
import 'package:mag_api_client/src/model/media_upload_intent_response_dto.dart';
import 'package:mag_api_client/src/model/merge_cart_dto.dart';
import 'package:mag_api_client/src/model/notification_preference_response_dto.dart';
import 'package:mag_api_client/src/model/notification_response_dto.dart';
import 'package:mag_api_client/src/model/notification_user_brief_dto.dart';
import 'package:mag_api_client/src/model/order_address_dto.dart';
import 'package:mag_api_client/src/model/order_applied_promotion_dto.dart';
import 'package:mag_api_client/src/model/order_brief_dto.dart';
import 'package:mag_api_client/src/model/order_line_item_dto.dart';
import 'package:mag_api_client/src/model/order_refund_dto.dart';
import 'package:mag_api_client/src/model/order_return_request_dto.dart';
import 'package:mag_api_client/src/model/order_return_request_item_dto.dart';
import 'package:mag_api_client/src/model/order_shipment_dto.dart';
import 'package:mag_api_client/src/model/order_shipment_item_dto.dart';
import 'package:mag_api_client/src/model/order_shipping_method_details_dto.dart';
import 'package:mag_api_client/src/model/paginated_audit_logs_dto.dart';
import 'package:mag_api_client/src/model/paginated_catalog_products_dto.dart';
import 'package:mag_api_client/src/model/paginated_notifications_dto.dart';
import 'package:mag_api_client/src/model/paginated_payment_attempts_dto.dart';
import 'package:mag_api_client/src/model/paginated_payment_webhook_events_dto.dart';
import 'package:mag_api_client/src/model/paginated_refunds_dto.dart';
import 'package:mag_api_client/src/model/paginated_report_exports_dto.dart';
import 'package:mag_api_client/src/model/paginated_reviews_dto.dart';
import 'package:mag_api_client/src/model/paginated_stock_movements_dto.dart';
import 'package:mag_api_client/src/model/pagination_dto.dart';
import 'package:mag_api_client/src/model/password_reset_confirm_response_dto.dart';
import 'package:mag_api_client/src/model/password_reset_request_response_dto.dart';
import 'package:mag_api_client/src/model/payment_attempt_response_dto.dart';
import 'package:mag_api_client/src/model/payment_webhook_event_response_dto.dart';
import 'package:mag_api_client/src/model/pricing_preview_applied_promotion_dto.dart';
import 'package:mag_api_client/src/model/pricing_preview_line_item_dto.dart';
import 'package:mag_api_client/src/model/pricing_preview_response_dto.dart';
import 'package:mag_api_client/src/model/pricing_preview_shipping_method_dto.dart';
import 'package:mag_api_client/src/model/product_translation_dto.dart';
import 'package:mag_api_client/src/model/product_type_attribute_link_dto.dart';
import 'package:mag_api_client/src/model/product_variant_dto.dart';
import 'package:mag_api_client/src/model/product_variant_translation_dto.dart';
import 'package:mag_api_client/src/model/promotion_performance_dto.dart';
import 'package:mag_api_client/src/model/promotion_performance_sum_dto.dart';
import 'package:mag_api_client/src/model/promotion_response_dto.dart';
import 'package:mag_api_client/src/model/public_legal_reference_dto.dart';
import 'package:mag_api_client/src/model/push_device_response_dto.dart';
import 'package:mag_api_client/src/model/receive_return_request_dto.dart';
import 'package:mag_api_client/src/model/receive_return_request_item_dto.dart';
import 'package:mag_api_client/src/model/refresh_token_dto.dart';
import 'package:mag_api_client/src/model/refund_response_dto.dart';
import 'package:mag_api_client/src/model/register_dto.dart';
import 'package:mag_api_client/src/model/register_push_device_dto.dart';
import 'package:mag_api_client/src/model/register_response_dto.dart';
import 'package:mag_api_client/src/model/report_export_download_response_dto.dart';
import 'package:mag_api_client/src/model/report_export_response_dto.dart';
import 'package:mag_api_client/src/model/report_export_user_brief_dto.dart';
import 'package:mag_api_client/src/model/reporting_count_metric_dto.dart';
import 'package:mag_api_client/src/model/request_password_reset_dto.dart';
import 'package:mag_api_client/src/model/reset_password_dto.dart';
import 'package:mag_api_client/src/model/return_request_item_response_dto.dart';
import 'package:mag_api_client/src/model/return_request_response_dto.dart';
import 'package:mag_api_client/src/model/review_moderation_action_dto.dart';
import 'package:mag_api_client/src/model/review_moderation_dto.dart';
import 'package:mag_api_client/src/model/review_order_line_dto.dart';
import 'package:mag_api_client/src/model/review_product_dto.dart';
import 'package:mag_api_client/src/model/review_product_translation_dto.dart';
import 'package:mag_api_client/src/model/review_response_dto.dart';
import 'package:mag_api_client/src/model/review_return_request_dto.dart';
import 'package:mag_api_client/src/model/sales_summary_amount_sum_dto.dart';
import 'package:mag_api_client/src/model/sales_summary_lines_metric_dto.dart';
import 'package:mag_api_client/src/model/sales_summary_lines_sum_dto.dart';
import 'package:mag_api_client/src/model/sales_summary_metric_dto.dart';
import 'package:mag_api_client/src/model/sales_summary_refunds_metric_dto.dart';
import 'package:mag_api_client/src/model/sales_summary_refunds_sum_dto.dart';
import 'package:mag_api_client/src/model/sales_summary_response_dto.dart';
import 'package:mag_api_client/src/model/serialized_order_dto.dart';
import 'package:mag_api_client/src/model/shipping_method_response_dto.dart';
import 'package:mag_api_client/src/model/shipping_zone_response_dto.dart';
import 'package:mag_api_client/src/model/staff_detail_dto.dart';
import 'package:mag_api_client/src/model/staff_status_response_dto.dart';
import 'package:mag_api_client/src/model/staff_summary_dto.dart';
import 'package:mag_api_client/src/model/stock_adjustment_response_dto.dart';
import 'package:mag_api_client/src/model/stock_level_response_dto.dart';
import 'package:mag_api_client/src/model/stock_level_variant_dto.dart';
import 'package:mag_api_client/src/model/stock_level_warehouse_dto.dart';
import 'package:mag_api_client/src/model/stock_movement_actor_dto.dart';
import 'package:mag_api_client/src/model/stock_movement_item_dto.dart';
import 'package:mag_api_client/src/model/stock_movement_variant_dto.dart';
import 'package:mag_api_client/src/model/stock_movement_warehouse_dto.dart';
import 'package:mag_api_client/src/model/stock_reservation_response_dto.dart';
import 'package:mag_api_client/src/model/stock_reservation_user_dto.dart';
import 'package:mag_api_client/src/model/stock_reservation_variant_dto.dart';
import 'package:mag_api_client/src/model/stock_reservation_warehouse_dto.dart';
import 'package:mag_api_client/src/model/store_setting_response_dto.dart';
import 'package:mag_api_client/src/model/tax_class_response_dto.dart';
import 'package:mag_api_client/src/model/top_product_dto.dart';
import 'package:mag_api_client/src/model/top_product_sum_dto.dart';
import 'package:mag_api_client/src/model/translation_dto.dart';
import 'package:mag_api_client/src/model/unregister_push_device_response_dto.dart';
import 'package:mag_api_client/src/model/update_admin_role_permissions_dto.dart';
import 'package:mag_api_client/src/model/update_cart_item_dto.dart';
import 'package:mag_api_client/src/model/update_legal_references_dto.dart';
import 'package:mag_api_client/src/model/update_pricing_behavior_dto.dart';
import 'package:mag_api_client/src/model/update_product_dto.dart';
import 'package:mag_api_client/src/model/update_product_variant_dto.dart';
import 'package:mag_api_client/src/model/update_shipment_status_dto.dart';
import 'package:mag_api_client/src/model/update_staff_roles_dto.dart';
import 'package:mag_api_client/src/model/update_staff_status_dto.dart';
import 'package:mag_api_client/src/model/update_store_currency_dto.dart';
import 'package:mag_api_client/src/model/upsert_content_page_dto.dart';
import 'package:mag_api_client/src/model/upsert_coupon_dto.dart';
import 'package:mag_api_client/src/model/upsert_manual_tax_rate_dto.dart';
import 'package:mag_api_client/src/model/upsert_notification_preference_dto.dart';
import 'package:mag_api_client/src/model/upsert_promotion_dto.dart';
import 'package:mag_api_client/src/model/upsert_shipping_method_dto.dart';
import 'package:mag_api_client/src/model/upsert_shipping_zone_dto.dart';
import 'package:mag_api_client/src/model/upsert_tax_class_dto.dart';
import 'package:mag_api_client/src/model/upsert_variant_price_dto.dart';
import 'package:mag_api_client/src/model/user_profile_dto.dart';
import 'package:mag_api_client/src/model/user_role_dto.dart';
import 'package:mag_api_client/src/model/variant_option_value_dto.dart';
import 'package:mag_api_client/src/model/variant_price_response_dto.dart';
import 'package:mag_api_client/src/model/verification_request_response_dto.dart';
import 'package:mag_api_client/src/model/verify_email_dto.dart';
import 'package:mag_api_client/src/model/verify_email_response_dto.dart';
import 'package:mag_api_client/src/model/wishlist_item_response_dto.dart';
import 'package:mag_api_client/src/model/wishlist_response_dto.dart';

part 'serializers.g.dart';

@SerializersFor([
  AddCartItemDto,
  AddWishlistItemDto,
  AdjustStockDto,
  AdminAccessCheckResponseDto,
  AdminCatalogAttributeDto,
  AdminCatalogAttributeOptionDto,
  AdminCatalogBrandDto,
  AdminCatalogCategoryDto,
  AdminCatalogCollectionDto,
  AdminCatalogMediaDto,
  AdminCatalogProductDto,
  AdminCatalogProductTypeAttributeDto,
  AdminCatalogProductTypeDto,
  AdminCatalogProductVariantOptionValueDto,
  AdminCatalogRelationLinkDto,
  AdminCatalogTagDto,
  AdminCatalogTranslationDto,
  AdminCatalogVariantDto,
  AdminCustomerSummaryDto,
  AdminLegalReferencesResponseDto,
  AdminPaginatedCatalogProductsDto,
  AdminPermissionDto,
  AdminRoleDto,
  ApplyCouponDto,
  AttachMediaDto,
  AttachedMediaResponseDto,
  AttachedMediaTranslationDto,
  AttributeOptionDto,
  AttributeOptionTranslationDto,
  AttributeTranslationDto,
  AuditLogActorDto,
  AuditLogResponseDto,
  AuthResponseDto,
  AuthUserDto,
  CancelOrderDto,
  CartItemResponseDto,
  CartResponseDto,
  CatalogLocalizedReferenceDto,
  CatalogMediaSummaryDto,
  CatalogProductSummaryDto,
  CatalogStatusResponseDto,
  CatalogVariantOptionSummaryDto,
  CatalogVariantPriceDto,
  CatalogVariantSummaryDto,
  CheckoutAddressDto,
  CheckoutCartPreviewDto,
  CheckoutPaymentAttemptDto,
  CheckoutPaymentReadinessDto,
  CheckoutPlaceOrderDto,
  CheckoutPlaceOrderResponseDto,
  CheckoutPreviewAppliedPromotionDto,
  CheckoutPreviewDto,
  CheckoutPreviewItemDto,
  CheckoutPreviewItemResponseDto,
  CheckoutPreviewItemStockDto,
  CheckoutPreviewResponseDto,
  CheckoutPreviewShippingMethodDto,
  ContentPageResponseDto,
  CouponResponseDto,
  CreateAdminRoleDto,
  CreateAttributeDto,
  CreateBrandDto,
  CreateCartDto,
  CreateCategoryDto,
  CreateCollectionDto,
  CreateMediaUploadIntentDto,
  CreateProductDto,
  CreateProductTypeDto,
  CreateProductVariantDto,
  CreateReportExportDto,
  CreateReturnRequestDto,
  CreateReturnRequestItemDto,
  CreateReviewDto,
  CreateShipmentDto,
  CreateShipmentItemDto,
  CreateTagDto,
  ExecuteReturnRefundDto,
  FulfillmentItemDto,
  FulfillmentResponseDto,
  HealthIndicatorDetailDto,
  HealthLivenessResponseDto,
  HealthReadinessResponseDto,
  InventoryKpisResponseDto,
  InventoryKpisStockMetricDto,
  InventoryKpisStockSumDto,
  InvitationSentResponseDto,
  InviteStaffDto,
  LegalReferencesResponseDto,
  LoginDto,
  LogoutDto,
  ManualTaxRateResponseDto,
  MediaTranslationDto,
  MediaUploadHeadersDto,
  MediaUploadIntentResponseDto,
  MergeCartDto,
  NotificationPreferenceResponseDto,
  NotificationResponseDto,
  NotificationUserBriefDto,
  OrderAddressDto,
  OrderAppliedPromotionDto,
  OrderBriefDto,
  OrderLineItemDto,
  OrderRefundDto,
  OrderReturnRequestDto,
  OrderReturnRequestItemDto,
  OrderShipmentDto,
  OrderShipmentItemDto,
  OrderShippingMethodDetailsDto,
  PaginatedAuditLogsDto,
  PaginatedCatalogProductsDto,
  PaginatedNotificationsDto,
  PaginatedPaymentAttemptsDto,
  PaginatedPaymentWebhookEventsDto,
  PaginatedRefundsDto,
  PaginatedReportExportsDto,
  PaginatedReviewsDto,
  PaginatedStockMovementsDto,
  PaginationDto,
  PasswordResetConfirmResponseDto,
  PasswordResetRequestResponseDto,
  PaymentAttemptResponseDto,
  PaymentWebhookEventResponseDto,
  PricingPreviewAppliedPromotionDto,
  PricingPreviewLineItemDto,
  PricingPreviewResponseDto,
  PricingPreviewShippingMethodDto,
  ProductTranslationDto,
  ProductTypeAttributeLinkDto,
  ProductVariantDto,
  ProductVariantTranslationDto,
  PromotionPerformanceDto,
  PromotionPerformanceSumDto,
  PromotionResponseDto,
  PublicLegalReferenceDto,
  PushDeviceResponseDto,
  ReceiveReturnRequestDto,
  ReceiveReturnRequestItemDto,
  RefreshTokenDto,
  RefundResponseDto,
  RegisterDto,
  RegisterPushDeviceDto,
  RegisterResponseDto,
  ReportExportDownloadResponseDto,
  ReportExportResponseDto,
  ReportExportUserBriefDto,
  ReportingCountMetricDto,
  RequestPasswordResetDto,
  ResetPasswordDto,
  ReturnRequestItemResponseDto,
  ReturnRequestResponseDto,
  ReviewModerationActionDto,
  ReviewModerationDto,
  ReviewOrderLineDto,
  ReviewProductDto,
  ReviewProductTranslationDto,
  ReviewResponseDto,
  ReviewReturnRequestDto,
  SalesSummaryAmountSumDto,
  SalesSummaryLinesMetricDto,
  SalesSummaryLinesSumDto,
  SalesSummaryMetricDto,
  SalesSummaryRefundsMetricDto,
  SalesSummaryRefundsSumDto,
  SalesSummaryResponseDto,
  SerializedOrderDto,
  ShippingMethodResponseDto,
  ShippingZoneResponseDto,
  StaffDetailDto,
  StaffStatusResponseDto,
  StaffSummaryDto,
  StockAdjustmentResponseDto,
  StockLevelResponseDto,
  StockLevelVariantDto,
  StockLevelWarehouseDto,
  StockMovementActorDto,
  StockMovementItemDto,
  StockMovementVariantDto,
  StockMovementWarehouseDto,
  StockReservationResponseDto,
  StockReservationUserDto,
  StockReservationVariantDto,
  StockReservationWarehouseDto,
  StoreSettingResponseDto,
  TaxClassResponseDto,
  TopProductDto,
  TopProductSumDto,
  TranslationDto,
  UnregisterPushDeviceResponseDto,
  UpdateAdminRolePermissionsDto,
  UpdateCartItemDto,
  UpdateLegalReferencesDto,
  UpdatePricingBehaviorDto,
  UpdateProductDto,
  UpdateProductVariantDto,
  UpdateShipmentStatusDto,
  UpdateStaffRolesDto,
  UpdateStaffStatusDto,
  UpdateStoreCurrencyDto,
  UpsertContentPageDto,
  UpsertCouponDto,
  UpsertManualTaxRateDto,
  UpsertNotificationPreferenceDto,
  UpsertPromotionDto,
  UpsertShippingMethodDto,
  UpsertShippingZoneDto,
  UpsertTaxClassDto,
  UpsertVariantPriceDto,
  UserProfileDto,
  UserRoleDto,
  VariantOptionValueDto,
  VariantPriceResponseDto,
  VerificationRequestResponseDto,
  VerifyEmailDto,
  VerifyEmailResponseDto,
  WishlistItemResponseDto,
  WishlistResponseDto,
])
Serializers serializers = (_$serializers.toBuilder()
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(PromotionResponseDto)]),
        () => ListBuilder<PromotionResponseDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(TopProductDto)]),
        () => ListBuilder<TopProductDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(SerializedOrderDto)]),
        () => ListBuilder<SerializedOrderDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AdminCatalogProductTypeDto)]),
        () => ListBuilder<AdminCatalogProductTypeDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(CartResponseDto)]),
        () => ListBuilder<CartResponseDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(WishlistResponseDto)]),
        () => ListBuilder<WishlistResponseDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AdminCatalogBrandDto)]),
        () => ListBuilder<AdminCatalogBrandDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AdminCatalogCategoryDto)]),
        () => ListBuilder<AdminCatalogCategoryDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ContentPageResponseDto)]),
        () => ListBuilder<ContentPageResponseDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AdminCustomerSummaryDto)]),
        () => ListBuilder<AdminCustomerSummaryDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AdminCatalogTagDto)]),
        () => ListBuilder<AdminCatalogTagDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(FulfillmentResponseDto)]),
        () => ListBuilder<FulfillmentResponseDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(StockLevelResponseDto)]),
        () => ListBuilder<StockLevelResponseDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AdminCatalogCollectionDto)]),
        () => ListBuilder<AdminCatalogCollectionDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AdminPermissionDto)]),
        () => ListBuilder<AdminPermissionDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(StockReservationResponseDto)]),
        () => ListBuilder<StockReservationResponseDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(PromotionPerformanceDto)]),
        () => ListBuilder<PromotionPerformanceDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ReturnRequestResponseDto)]),
        () => ListBuilder<ReturnRequestResponseDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AdminRoleDto)]),
        () => ListBuilder<AdminRoleDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(StaffSummaryDto)]),
        () => ListBuilder<StaffSummaryDto>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AdminCatalogAttributeDto)]),
        () => ListBuilder<AdminCatalogAttributeDto>(),
      )
      ..add(const OneOfSerializer())
      ..add(const AnyOfSerializer())
      ..add(const DateSerializer())
      ..add(Iso8601DateTimeSerializer())
    ).build();

Serializers standardSerializers =
    (serializers.toBuilder()..addPlugin(StandardJsonPlugin())).build();
